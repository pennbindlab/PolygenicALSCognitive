...
CReATe_data <- read.csv("data/CReATe_data.csv", stringsAsFactors = FALSE)
...
bl_ECAS_Verb <=14 | bl_ECAS_Exec <=33 | bl_ECAS_Lang <=26 ~ "ALSci",
  ...
  select(ID, starts_with("bl_"), ClinDx2) %>% 
...
write.csv(CReATe_data, "cleaned_CReATe_data_long.csv")
```
...
write.csv(cleaned_CReATe_data, "cleaned_CReATe_data.csv")
```
...
Plot principal components 1 and 2 from the PCA conducted in the CReATe PGB cohort (Supplementary Figure 1).
