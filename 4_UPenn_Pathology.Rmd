...
weights <- read.csv("data/wPRS_weights.csv") %>% 
  ...
  filter(!Features %in% c("Principal Component 1","Principal Component 2","rs142321490"))
...
Table_2B <-  UPenn_path_data %>% 
  ...
  mutate(Mutation_Summary = as.factor(Mutation_Summary)) %>% 
  CreateTableOne(vars = c("Sex", "NPDx1", "Race", "Ethnicity", "AgeatDeath", "DiseaseDuration", "ALSSymptomOnsetSite", "Mutation_Summary"), data = .)
...
Create boxplots for magnitude of Neuronal Loss in each region plotted against wPRS (Supplementary Figure 10). 
```{r Supplementary_Figure_10}
pdf(file='Supplementary_Figure_10.pdf')
UPenn_path_data_ordinal %>% 
...
Create boxplots for magnitude of TDP-43 pathology in each region plotted against wPRS (Supplementary Figure 11).
```{r Supplementary_Figure_11}
pdf(file='Supplementary_Figure_11.pdf')
UPenn_path_data_ordinal %>% 
